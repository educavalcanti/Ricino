...
<form action="<?php echo url_for('News/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?noti_cd_chave='.$form->getObject()->getNotiCdChave() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<table class="adminTable newTable">
    <tfoot>
      <tr>
        <td colspan="2">
          ...
          &nbsp;<a href="<?php echo url_for('News/index') ?>">Voltar para as Notícias</a>
          <?php if (!$form->getObject()->isNew()): ?>
            &nbsp;<?php echo link_to('Remover', 'News/delete?noti_cd_chave='.$form->getObject()->getNotiCdChave(), array('method' => 'delete', 'confirm' => 'Are you sure?')) ?>
          <?php endif; ?>
    ...
    <tbody>
      ...
      <!--<tr>
        <th><?php //echo $form['tino_cd_chave']->renderLabel() ?></th>
        <td>
          <?php //echo $form['tino_cd_chave']->renderError() ?>
          <?php //echo $form['tino_cd_chave'] ?>
        </td>
      </tr>
      -->
      <tr>
